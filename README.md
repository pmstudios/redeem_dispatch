...
- **모바일 대응 추가** — 원본은 `.frame` 이 `width:780px` 고정에 미디어 쿼리가 0개라,
  390px 폰에서 내용의 절반이 화면 밖으로 잘렸습니다. `App.css` 맨 끝에 반응형 블록을
  추가했습니다. **기존 규칙은 건드리지 않고 뒤에서 덮어쓰는 방식**이라 데스크톱 화면은
  동일합니다. `h1` 의 `line-height:0.46` 은 한 줄 전제라, 제목이 접히는 좁은 화면에서는
  `1.25` 로 올려야 줄이 안 겹칩니다.
